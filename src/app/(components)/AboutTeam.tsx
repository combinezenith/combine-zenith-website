...
export default function MeetTheTeam() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Animate team member cards with varied entrance animations
      if (!loading && gridRef.current) {
        const cards = gridRef.current.querySelectorAll('.team-card');
        if (cards.length > 0) {
          // Different animation patterns for variety
          const animations = [
            { x: -100, y: 0, rotation: -10 },    // Slide from left with slight rotation
            { x: 100, y: 0, rotation: 10 },     // Slide from right with slight rotation
            { x: 0, y: -100, rotation: 0 },     // Slide from top
            { x: -50, y: -50, rotation: -5 },   // Diagonal from top-left
            { x: 50, y: -50, rotation: 5 },     // Diagonal from top-right
            { x: 0, y: 100, rotation: 0 },      // Slide from bottom
          ];

          cards.forEach((card, index) => {
            const animationIndex = index % animations.length;
            const animation = animations[animationIndex];
            const delay = index * 0.1; // Stagger with smaller delay
    ...
    <section ref={sectionRef} aria-label="Meet The Team Section" className="relative py-20 overflow-hidden">
      ...
      <div aria-label="Team Container" className="container mx-auto px-6 relative z-10">
        <div aria-label="Team Content" className="max-w-6xl mx-auto">
          {/* Header Section */}
          <div ref={headerRef} aria-label="Team Header" className="text-center mb-16">
          ...
          {/* Team Grid */}
          <div ref={gridRef} aria-label="Team Members Grid" className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {teamMembers.map((member, index) => (
              <div
                key={index}
                className="team-card bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10 hover:border-purple-400/30 transition-all duration-300 group text-center hover:transform hover:scale-105 hover:shadow-lg hover:shadow-purple-500/25"
              >
                {/* Profile Image */}
                {member.image ? (
                  <Image
