...
export async function generateStaticParams() {
  return (portfolio as PortfolioItem[]).map((p) => ({ id: p.slug }));
}

export default function PortfolioDetail({ params }: Params) {
  const id = params.id;
  const item = (portfolio as PortfolioItem[]).find((p) => p.slug === id);
          ...
          />
            ...
            <div className=" rounded-xl flex flex-col md:flex-row items-center md:items-center">
              ...
              <div className="w-full md:w-1/2 px-6 py-8 text-center md:text-left animate-slideInRight">
                ...
                <p className="mt-4 text-gray-300 text-sm sm:text-base">{item.overview}</p>
              </div>
            </div>
